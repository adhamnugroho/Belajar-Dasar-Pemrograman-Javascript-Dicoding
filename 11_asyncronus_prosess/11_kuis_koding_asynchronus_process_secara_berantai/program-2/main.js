...
async function getTollAccess(money, amountTopUp) {
  try {
    const tollRoadCard = await buyTollRoadCard(money);
    const balance = await topUpBalance(tollRoadCard, amountTopUp);
    const accessTollRoad = await useTollRoad(balance);

    return accessTollRoad;
  } catch (error) {
    throw error;
  }
}
...
getTollAccess(25, 10).catch((error) => console.log(error.message));
